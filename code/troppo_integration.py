...
def reconstruction_pipeline():
    ...
    ***config_variables.py***, and ***medium_variables.py files***.
    ...
    for condition in omics_data.index:
        # thresholds[condition] = {0.55: 5*log(2)}
        thresholds[condition]= {quantile: omics_data.loc[condition].quantile(quantile).round(3) for quantile in linspace(0.10, 0.20, 21)}
    print(thresholds)
    ...
    for algorithm in params["ALGORITHMS"]:
        ...
        for condition, cond_thresholds in thresholds.items():
            ...
            for quantile, threshold in cond_thresholds.items():
                troppo_result = troppo_omics_integration(model=template_model, algorithm=algorithm, threshold=threshold,
                                                         thread_number=thred_number, omics_dataset=omics_data.loc[[sample_name]],
                                                         params=params)

                for sample in list(troppo_result.keys()):
                    th = str(round(quantile, 2))
        ...
        if params["THRESHOLDING_STRATEGY"] == 'default':
            ...
            file_path = f'{algorithm}_{params["THRESHOLDING_STRATEGY"]}_' \
